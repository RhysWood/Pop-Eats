  ...
  database
    ...
    .then((user) => {
      if (user.is_owner) {
        ...
        database.setCompleted(req.params.orderID).then((result) => {
          database.getUserFromOrder(req.params.orderID).then((user) => {
...
app.post("/update-menu/update/:itemID", (req, res) => {
  ...
  database
    ...
    .then((user) => {
      if (user.is_owner) {
        database.editMenuItem(req.params.itemID, req.body).then(() => {
          res.redirect("/update-menu");
        });
      } else {
        return res.status(401).send("error, wrong user");
